...
from platform_config import (
    COMPATIBILITY_LABEL,
    EXAMPLE_PLATFORM_URL,
    EXAMPLE_UNIVERSAL_URL,
    TARGET_PLATFORM_LABEL,
    TARGET_RUNTIME_LABEL,
    is_target_linux_wheel,
    is_universal_wheel,
)
...
def determine_package_type(file_name: str) -> str:
    ...
    if is_universal_wheel(file_name):
...
def append_usage_sections(lines: list[str]) -> None:
    ...
    lines.append(f"All packages in this repository require Python 3.13.x on {TARGET_PLATFORM_LABEL} for compatibility.")
    lines.append("")
    lines.append("### Linux Installation")
    lines.append("")
    lines.append("Currently these are all x86_64 packages, not x86 (32-bit).")
    lines.append("")
    lines.append("#### Ubuntu Packages")
    lines.append("")
    lines.append("Install or activate Python 3.13.x on Ubuntu before installing packages from this feed.")
    lines.append("")
    lines.append("```bash")
    lines.append("python3.13 -m venv .venv")
    ...
    lines.append("```")
    lines.append(EXAMPLE_UNIVERSAL_URL)
    lines.append(EXAMPLE_PLATFORM_URL)
    lines.append("```")
    lines.append("")
...
def render_quick_stats(manifest: dict, lines: list[str]) -> None:
    ...
    lines.append(f"- **Target runtime:** {TARGET_RUNTIME_LABEL}")
    lines.append("")
...
def render_readme(manifest: dict) -> str:
    ...
    lines.append("")
    lines.append(f"> ⚠️ **Compatibility:** {COMPATIBILITY_LABEL}")
    lines.append("")
    ...
    lines.append(f"- **{TARGET_PLATFORM_LABEL}** environment")
    lines.append("")
