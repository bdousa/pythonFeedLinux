...
from platform_config import user_agent  # noqa: E402
...
SYSTEM_PROMPT = """You are an advisory security reviewer for a Python package approval pipeline that feeds a Linux-focused internal package mirror. Your job is to triage Snyk findings and package metadata so a human approver can quickly understand the real risk.
...
def call_foundry(endpoint: str, api_key: str, model: str, evidence: dict) -> tuple[str, str]:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "api-key": api_key,
            "User-Agent": user_agent("AI Security Reviewer"),
        },
        method="POST",
    )
    try:
