...
from platform_config import (
    NON_TARGET_OS_CLASSIFIER_TOKENS,
    TARGET_OS_CLASSIFIER_TOKENS,
    TARGET_PLATFORM_LABEL,
    user_agent,
)

USER_AGENT = user_agent("Review Bot")
...
def evaluate_os_compatibility(classifiers: list[str]) -> dict:
    ...
    summary["linuxCompatible"] = any(
        any(token in c for token in TARGET_OS_CLASSIFIER_TOKENS) for c in os_classifiers
    )

    has_other_os = any(any(token in c for token in NON_TARGET_OS_CLASSIFIER_TOKENS) for c in os_classifiers)
    only_other_os = has_other_os and not summary["linuxCompatible"] and not summary["osIndependent"]
    summary["otherOsOnly"] = only_other_os

    if summary["osIndependent"] or summary["linuxCompatible"]:
        summary["status"] = "pass"
        if has_other_os and not summary["osIndependent"]:
            summary["notes"].append("Declares other OS classifiers alongside Linux; verify the chosen artifact is the Linux build")
    elif only_other_os:
        summary["status"] = "block"
        summary["notes"].append("Package declares only non-Linux operating systems")
    else:
        summary["status"] = "review"
        summary["notes"].append("Could not confirm Linux compatibility from classifiers")
    return summary
...
def build_recommendation(dep_counts, code_counts, last_release_days, license_status, os_status, snyk_statuses):
    ...
    if os_status == "block":
        reasons.append("Package is not compatible with the Linux feed")
        return "reject", reasons
...
def render_markdown(report: dict) -> str:
    ...
    lines.append(f"- Prerequisites: Python 3.13.x, {TARGET_PLATFORM_LABEL}")
    lines.append("")
    ...
    lines.append("- OK: Packages with Linux, POSIX, or Unix classifiers")
    lines.append("- Review: Packages without any OS classifiers")
    lines.append("- Block: Packages that declare only non-Linux operating systems")
    lines.append("")
    lines.append(f"This package: **{os_status_label(os_compat['status'])}**")
    if os_compat['osIndependent']:
        lines.append("- OS Independent: yes")
    if os_compat['linuxCompatible']:
        lines.append("- Linux compatible: yes")
    if os_compat['otherOsOnly']:
        lines.append("- Declares only non-Linux operating systems")
